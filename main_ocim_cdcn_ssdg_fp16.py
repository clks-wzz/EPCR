...
from models.simsiam_semi_cdcn_meanteacher_SSDG import Discriminator_FP16
...
def main(args):
    ...
    if args.amp:
        ...
        print("===> Use FP16 training...")

    train_set, train_sampler = get_dataset_liveness(
        args.dataset, 
        args.data_dir, 
    ...
    # define model
    loss_softmax = nn.CrossEntropyLoss(reduction='none').cuda()
    discriminator = Discriminator_FP16().cuda()
    ...
    optimizer_dict = [
        {"params": filter(lambda p: p.requires_grad, model.parameters()), "lr": args.base_lr*args.batch_size/256},
        {"params": filter(lambda p: p.requires_grad, discriminator.parameters()), "lr": args.base_lr*args.batch_size/256},
    ]

    if args.optimizer == 'sgd':
        optimizer = torch.optim.SGD(optimizer_dict, lr=args.base_lr*args.batch_size/256, momentum=args.momentum, weight_decay=args.weight_decay)
    elif args.optimizer == 'adam':
        optimizer = torch.optim.Adam(optimizer_dict, lr=args.base_lr*args.batch_size/256, weight_decay=args.weight_decay)
    else:
        raise ValueError
    ...
    losses_ssdg = AverageMeter('Loss-ssdg', ':.6f')
    ...
    for epoch in range(args.stop_at_epoch):
        ...
        progress = ProgressMeter(
            len(train_loader),
            [batch_time, data_time, losses_us, losses_ce, losses_ssdg, losses_tt, top1, learning_rate],
        ...
        for idx, ((images1, images2), labels, domains, triplets) in enumerate(train_loader):
            labels = labels.to(args.device)
            domains = domains.to(args.device)
            triplets = triplets.to(args.device)
            ...
            with torch.cuda.amp.autocast():
                losses, (c1, c2) = model.forward(images1, images2, labels, domains, triplets)

                d1_avg = losses['d1_avg']
                d1 = discriminator(d1_avg)
                real_adloss = loss_softmax(d1, domains)
                real_adloss = real_adloss.squeeze() * (1 - labels) 
                real_adloss = torch.sum(real_adloss) / torch.sum(1 - labels)

                triplet_loss = losses['loss_triplet']

                # try0=0.0, try3=0.01, try5=0.1, try6=0.5
                losses['loss_tt'] += args.loss_weight_triplet * triplet_loss +  args.loss_weight_adloss * real_adloss

                for key in losses.keys():
                    losses[key] = losses[key].sum() / float(torch.cuda.device_count())

            if args.amp:
                scaler.scale(losses['loss_tt']).backward()
            ...
            losses_ssdg.update((losses['loss_triplet'] - real_adloss).item())
