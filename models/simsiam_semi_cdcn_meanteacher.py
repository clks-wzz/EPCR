...
def D_Dense(p, z, version='original'): # negative cosine similarity
    ...
    elif version == 'position2position':
        z = z.detach() # stop gradient
        p = F.normalize(p, dim=1) # l2-normalize 
        z = F.normalize(z, dim=1) # l2-normalize 
        loss = p * z 
    ...
    elif version == 'norm2norm':
        z = z.detach() # stop gradient
        p = F.normalize(p, dim=1) # l2-normalize 
        z = F.normalize(z, dim=1) # l2-normalize 
        p = _reshape(p, _t=False).mean(dim=-1)
        z = _reshape(z, _t=False).mean(dim=-1)
        p = F.normalize(p, dim=1) # l2-normalize 
        z = F.normalize(z, dim=1) # l2-normalize 
...
def D_DenseCL(feat_q, feat_k, dense_q, dense_k):
    n, c, h, w = feat_q.size()
...
class classify_MLP_Ensemble(nn.Module):
    def __init__(self, in_dim=64, hidden_dim=64*3, n_classes=1, ensemble_num=3): # bottleneck structure
        super().__init__()
        if hidden_dim > 0:
            self.layer1 = nn.Sequential(
                conv2d_1x1(in_dim, hidden_dim),
...
'''v11 normal classifier''' 
class SimSiam_Semi_CDCN_MeanTeacherV11(nn.Module):
    ...
    def postprocess_map(self, maps):
        ...
        v_sq = v.squeeze()
        if len(v_sq.shape) == 0:
            v = torch.tensor([v_sq]).cuda().unsqueeze(1)
        else:
    ...
    def inference_train(self, x1, x2, label, needs):
        ...
        #ce1 = MSE(c1, label) * needs
        ce1 = MSE(pz1, label) * needs / 2. + MSE(pz2, label) * needs / 2.
    ...
    def forward(self, x1, x2=None, label=None, needs=None):
        if not x2 is None:
            return self.inference_train(x1, x2, label, needs)
        else:
